...
contract('SupplyChain', function(accounts) {
    ...
    it("Testing roles", async() => {
        const supplyChain = await SupplyChain.deployed();
        await supplyChain.addFarmer(originFarmerID);
        await supplyChain.addDistributor(distributorID);
        await supplyChain.addRetailer(retailerID);
        await supplyChain.addConsumer(consumerID);
        assert.equal(await supplyChain.isFarmer(originFarmerID), true, 'Farmer role set failed') 
        assert.equal(await supplyChain.isDistributor(distributorID), true, 'Distributor role set failed') 
        assert.equal(await supplyChain.isRetailer(retailerID), true, 'Retailer role set failed') 
        assert.equal(await supplyChain.isConsumer(consumerID), true, 'Consumer role set failed') 
    })
